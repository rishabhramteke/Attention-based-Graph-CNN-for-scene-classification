...
class GraphCNNNetwork(object):
    ...
    def make_graph_attention_layer(self, F_, attn_heads, name=None, with_bn=False, with_act_func=False):
        print_ext("Attention Layer")
        print_ext("Shape of V:",self.current_V.get_shape())
        print_ext("no of nodes in output:",self.current_V.get_shape()[1].value)
        print_ext("no of features in output:",F_)
        with tf.variable_scope(None, default_name='attention') as scope:
            self.current_V = graph_attention(self.current_V, self.current_A, F_, attn_heads )
            if with_bn:
                self.make_batchnorm_layer()
            if with_act_func:
                self.current_V = tf.nn.relu(self.current_V)
